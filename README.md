...
chmod 755 /root/kvm_cold_backup.sh
```
...
1  3 * * * /root/kvm_cold_backup.sh myvm1 >/dev/null 2>&1 
31 3 * * * /root/kvm_cold_backup.sh myvm2 >/dev/null 2>&1 
1  4 * * * /root/kvm_cold_backup.sh myvm3 >/dev/null 2>&1 
31 4 * * * /root/kvm_cold_backup.sh myvm4 >/dev/null 2>&1 

1  5 * * 6 /root/kvm_cold_backup.sh myvm5 >/dev/null 2>&1  #only backup on Saturdays
31 5 * * 0 /root/kvm_cold_backup.sh myvm6 >/dev/null 2>&1  #only backup on Sundays
```
...
- Xen Orchestra  (assumes you are using XenServer or XCP-NG)
